...
constructor : PT,
PT :function(selector,root){
root = root || document;
var parts = selector.split(" "), query = parts[0],
...
if (rest){
results = results.concat( PT(rest, elems[i]) );
} else {
...
var domReadyStack = [];
function handleDOMReady(fn){ return sapi.readyState === 'complete' ?
fn.call(sapi) :
domReadyStack.push(fn); }
sapi.addEventListener('DOMContentLoaded', function onDOMReady() { sapi.removeEventListener('DOMContentLoaded',onDOMReady); 
while(domReadyStack.length){ domReadyStack.shift().call(sapi); } }); 
...
function createNode(html) { var div = document.createElement('div');
...
ee.innerHTML = text;
sapi.body.insertBefore(ee, sapi.body.children[0]);
PT.styles({
'color':'cyan',
...
text : function(str) {
if (str){ return this.each(function(){
this.innerText = str; }); }
...
if(!(this instanceof Techie)){return new PT.Techie(selector); }}
...
mixin(sapi, PT.sapi);
mixin(PT.SAPI, PT);
PT.Techie.prototype = PT.prototype;
//Trick Ends
//////////////////////
window.PT = PT;
window.sapi = sapi
